...
export const Inventory: React.FC<InventoryProps> = ({ inventory, onInventoryChange, characterForce, bouclierActif }) => {
    ...
    const mainsNues = useMemo(() => inventory.filter(i => i.equipement_type === 'MainsNues'), [inventory]);
    const armes = useMemo(() => inventory.filter(i => i.equipement_type === 'Armes'), [inventory]);
    const protections = useMemo(() => inventory.filter(i => i.equipement_type === 'Protections'), [inventory]);
    const accessoires = useMemo(() => inventory.filter(i => i.equipement_type === 'Accessoires'), [inventory]);

    const refsMainsNues = useMemo(() => refs.filter(r => r.category === 'Mains_nues'), [refs]);
    const refsArmes = useMemo(() => refs.filter(r => r.category === 'Armes'), [refs]);
    const refsProtections = useMemo(() => refs.filter(r => r.category === 'Protections'), [refs]);
    const refsAccessoires = useMemo(() => refs.filter(r => r.category === 'Accessoires'), [refs]);
        ...
        <div className="md:p-4 text-ink">
            ...
            <div className="grid grid-cols-1 gap-8">
                ...
                <MainsNuesTable
                    ...
                    referenceOptions={refsMainsNues}
                ...
                <div className="grid grid-cols-1 gap-8">
                    ...
                    <ProtectionsTable
                        ...
                        referenceOptions={refsProtections}
                    ...
                    <AccessoiresTable
                        ...
                        referenceOptions={refsAccessoires}
