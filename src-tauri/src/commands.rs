...
#[tauri::command]
pub fn get_docs_list(app: AppHandle) -> Result<Vec<String>, String> {
    let resource_dir = app.path().resource_dir().map_err(|e| e.to_string())?;
    let docs_path = resource_dir.join("data").join("docs");

    if !docs_path.exists() {
        return Ok(vec![]);
    }

    let entries = std::fs::read_dir(&docs_path).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn open_doc(filename: String, app: AppHandle) -> Result<(), String> {
    let resource_dir = app.path().resource_dir().map_err(|e| e.to_string())?;
    let doc_path = resource_dir.join("data").join("docs").join(&filename);

    if !doc_path.exists() {
        return Err(format!("Fichier introuvable: {}", filename));
    }

    std::process::Command::new("cmd")
        .args(["/C", "start", "", &doc_path.to_string_lossy()])
        .spawn()
